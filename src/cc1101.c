...
void cc1101_sendDataPollTxFifo(uint8_t *data, uint16_t numBytes)
{
    ...
    if (numBytes > 0)
    {
        ...
        cc1101_writeBurst(CC1101_TXFIFO, data, CC1101_TXFIFO_LEN);
        byteCounter += CC1101_TXFIFO_LEN;
        printf("TXBYTES %u\n", cc1101_readStatus(CC1101_TXBYTES) & 0x7F);
        cc1101_setTxState();
        //printf("MARCSTATE %u\n", cc1101_readStatus(CC1101_MARCSTATE) & 0x1F);
        do {
            // Check how many bytes left in TX FIFO
            uint8_t txbytes = cc1101_readStatus(CC1101_TXBYTES) & 0x7F;
            //printf("TXBYTES %u\n", txbytes);
            if (txbytes < CC1101_TXFIFO_LEN) {
                // Write next data into the TX FIFO
                //printf("Refill TX FIFO with %u bytes.\n", CC1101_TXFIFO_LEN-txbytes);
                cc1101_writeBurst(CC1101_TXFIFO, &data[byteCounter], CC1101_TXFIFO_LEN-txbytes);
                byteCounter += CC1101_TXFIFO_LEN-txbytes;
                //  printf("MARCSTATE %u\n", cc1101_readStatus(CC1101_MARCSTATE) & 0x1F);
            }
            else {
                if (txbytes > CC1101_TXFIFO_LEN) {
                    printf("Erroneous TXFIFO read or overflow. Go back to IDLE state.\n");
                    cc1101_flushTxFifo();
                    cc1101_setIdleState();
                    return;
                }
    ...
    do {
        // Wait for IDLE state
        printf("TXBYTES after TX FIFO write loop %u\n", cc1101_readStatus(CC1101_TXBYTES) & 0x7F);
        marcState = cc1101_readStatus(CC1101_MARCSTATE) & 0x1F;
        printf("MARCSTATE after TX FIFO write loop: %u\n", marcState);
    } while (IDLE != marcState);
}
